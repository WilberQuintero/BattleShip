...
public class controladorPartida {
     ...
     * @param partidaEntidad La entidad Partida ya procesada (convertida desde
     * DTO).
     * @param nombreJugadorLocal El nombre del jugador que está usando esta
    ...
    /**
     * Devuelve la instancia de la Partida actual que maneja este controlador.
     *
     * @return La entidad Partida actual.
     */
    ...
    /**
     * Configura la vista con la información inicial de la partida. Este método
     * reemplaza a tu antiguo "IniciarCombate" en términos de configuración de
     * UI.
     */
    ...
    /**
     * Configura los listeners en la vista para cuando el jugador haga clic en
    ...
    private void actualizarInformacionDeTurno() {
        if (vistaPartida == null || partidaActual == null || partidaActual.obtenerJugadorEnTurno() == null) {
            return;
        }
    ...
    public void procesarResultadoDisparo(Map<String, Object> datos) {
        ...
        try {
            ...
            e.printStackTrace();
            if (vistaPartida != null) {
                vistaPartida.mostrarError("Error procesando resultado del disparo: " + e.getMessage(), false);
            }
        }
    }
    ...
    public void procesarFinDePartida(Map<String, Object> datos) {
        ...
        if (vistaPartida == null || partidaActual == null) {
            System.err.println("CONTROLADOR_PARTIDA ERROR: Vista o Partida nula en procesarFinDePartida.");
            return;
        }

        String idSalaEvento = (String) datos.get("idSala");
        if (idSalaEvento == null || !idSalaEvento.equals(partidaActual.getIdPartida())) {
            System.err.println("CONTROLADOR_PARTIDA: FIN_DE_PARTIDA para sala incorrecta. Ignorando.");
            return;
        }

        final String nombreGanador = (String) datos.get("ganador");
        final String motivo = (String) datos.get("motivo");

        partidaActual.setEstado(EstadoPartida.FINALIZADA); // O ABANDONADA si el motivo lo indica
        partidaActual.setJugadorEnTurno(null);

        // Crear un mensaje más personalizado
        String mensajeResultadoFinal;
        boolean yoGane = false;
        if (nombreGanador != null && nombreGanador.equals(this.nombreJugadorLocal)) {
            mensajeResultadoFinal = "¡FELICIDADES, " + this.nombreJugadorLocal + "! Has ganado. Motivo: " + motivo;
            yoGane = true;
        } else if (nombreGanador != null) {
            mensajeResultadoFinal = "Has perdido. Ganador: " + nombreGanador + ". Motivo: " + motivo;
        } else {
            mensajeResultadoFinal = "Partida terminada. Motivo: " + motivo;
        }
        System.out.println(mensajeResultadoFinal);

        // Guardar la referencia a la vista antes de llamar a dispose en otro hilo
        final PantallaPartida vistaActual = this.vistaPartida;

        SwingUtilities.invokeLater(() -> {
            if (vistaActual != null && vistaActual.isDisplayable()) {
                vistaActual.dispose(); // Cierra PantallaPartida
            }

//            if (yoGane) {
//                System.out.println("CONTROLADOR_PARTIDA: Mostrando pantalla Victoria para " + this.nombreJugadorLocal);
//                Victoria pantallaVictoria = new Victoria();
//                // Aquí podrías pasar datos a la pantalla Victoria si su constructor lo permite:
//                // new Victoria(this.nombreJugadorLocal, motivo).setVisible(true);
//                pantallaVictoria.setVisible(true);
//            } else {
//                System.out.println("CONTROLADOR_PARTIDA: Mostrando pantalla Derrota para " + this.nombreJugadorLocal + ". Ganador fue: " + nombreGanador);
//                Derrota pantallaDerrota = new Derrota();
//                // Aquí podrías pasar datos a la pantalla Derrota:
//                // new Derrota(this.nombreJugadorLocal, nombreGanador, motivo).setVisible(true);
//                pantallaDerrota.setVisible(true);
//            }

            // Limpiar la referencia a este controlador y la partida en controladorInicio
//            if (ctrlInicioRef != null) {
//                ctrlInicioRef.eliminarPartida(this.idSala); // Elimina la partida del mapa de activas
//                ctrlInicioRef.limpiarControladorDePartidaActual(); // Esto debería poner ctrlInicioRef.controladorDePartidaActual a null
//            }
        });
    }
    ...
    public void abandonarPartida() {
        if (partidaActual == null || serverComunicacion == null || !serverComunicacion.isConectado()) {
            if (vistaPartida != null) {
                vistaPartida.mostrarError("No se puede abandonar, sin conexión o partida no válida.", false);
            }
            return;
        }
        System.out.println("CONTROLADOR_PARTIDA: Jugador " + nombreJugadorLocal + " solicita abandonar partida " + partidaActual.getIdPartida());

        String eventoAbandono = String.format("EVENTO;TIPO=ABANDONAR_PARTIDA;idSala=%s;nombreJugador=%s",
                partidaActual.getIdPartida(), nombreJugadorLocal);
        serverComunicacion.enviarEventoJuego(eventoAbandono); // Usar el método correcto

        if (vistaPartida != null) {
            // vistaPartida.mostrarMensajeGeneral("Solicitud de abandono enviada...");
            // No es necesario deshabilitar todo aquí, ya que el servidor responderá con FIN_PARTIDA
            // que cerrará la PantallaPartida y mostrará Victoria/Derrota.
        }
    }
